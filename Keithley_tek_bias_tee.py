...
def setup_vcm_plots():
    ...
    def _is_valid_data(series):
        """Helper to check if a pandas Series contains plottable data."""
        # It's valid if it's not None, and after removing NaNs, it's not empty.
        return series is not None and not pd.Series(series).dropna().empty

    def plot0(data, ax=None, **kwargs):
        ax.cla()
        ax.set_title('Pre Pulse Resistance State')
        if data.empty or 't_hrs' not in data.columns:
            return

        line = data.iloc[-1]
        i = 0
        # If the last row's data is invalid, try the one before it.
        if not _is_valid_data(line.t_hrs):
            i += 1
            if len(data) > 1:
                line = data.iloc[-2]
            else:
        ...
        # Final check on the selected line before plotting
        if _is_valid_data(line.t_hrs):
    ...
    def plot1(data, ax=None, **kwargs):
        ax.cla()
        ax.set_title('Pulse Answer')
        if data.empty or 't_ttx' not in data.columns:
            return

        line = data.iloc[-1]
        i = 0
        if not _is_valid_data(line.t_ttx):
            i += 1
            if len(data) > 1:
                line = data.iloc[-2]
            else:
    ...
    def plot2(data, ax=None, **kwargs):
        ax.cla()
        ax.set_title('Post Pulse Resistance State')
        if data.empty or 't_lrs' not in data.columns:
            return

        line = data.iloc[-1]
        i = 0
        if not _is_valid_data(line.t_lrs):
            i += 1
            if len(data) > 1:
                line = data.iloc[-2]
            else:
    ...
    def plot3(data, ax=None, **kwargs):
        ax.cla()
        ax.set_title('Sweep')
        if data.empty or 'V_sweep' not in data.columns:
            return

        line = data.iloc[-1]
        i = 0
        if not _is_valid_data(line.V_sweep):
            i += 1
            if len(data) > 1:
                line = data.iloc[-2]
            else:
        ...
        if _is_valid_data(line.V_sweep):
            resistance_sweep = (line.V_sweep / line.I_sweep) - 50
            if np.any(resistance_sweep > 0):
                ax.set_title('Sweep #' + str(len(data) - i))
                ax.semilogy(line.V_sweep, resistance_sweep)
...
def call_ppg_with_params(pname, pulse_pattern, is_delay, repetitions, last_pulse, vset, vreset, record):
    ...
    limitI = 1e-3,
    limitI2 = 1e-2,)
    resistance_val = res2[0]
    if resistance_val is None:
        # If measurement failed, return a value indicating failure
        return float('inf')
    # If we have a valid number, convert it to an integer
    resistance = int(resistance_val)
    return resistance
...
cap_value = 7_500_000):
    ...
    # ------------------ Input Validation ----------------------------------------------------
    if vset < 1.0:
        raise ValueError(f"Initial vset ({vset}V) is below the hardware limit of 1.0V. Please provide a valid starting vset.")
    if vset > 2.0:
        raise ValueError(f"Initial vset ({vset}V) is above the hardware limit of 2.0V. Please provide a valid starting vset.")
    if vreset < 0.6:
        raise ValueError(f"Initial vreset ({vreset}V) is below the hardware limit of 0.6V. Please provide a valid starting vreset.")
    if vreset > 1.2:
        raise ValueError(f"Initial vreset ({vreset}V) is above the hardware limit of 1.2V. Please provide a valid starting vreset.")

    # ---------------- Constants and variables -----------------------------------------------------

    padname        = pname                                    # name of the device
    overall_cycles = 0                                        # running total of cycles (P→N), monotonic
    successful_cycles = 0                                     # running total of successful cycles (P→N), monotonic
    ...
    HRS_THRESHOLD = 5500.0                                    # threshold value for HRS 
    LRS_THRESHOLD = 3500.0                                    # threshold value for LRS 
    HRS_OVERDRIVE_THRESHOLD = 50000.0                         # threshold value for HRS overdrive
    LRS_OVERDRIVE_THRESHOLD = 500.0                           # threshold value for LRS overdrive 
    ...
    while True:
        ...
        if r >= HRS_THRESHOLD:
            ...
            def _log_round(phase: str, num_cycles: int, r_val: float, used_reps: int, capped: bool):
                ...
                """
                nonlocal successful_cycles 

                is_successful = False
                if phase == "SET" and r_val <= LRS_THRESHOLD:
                    is_successful = True
                elif phase == "RESET" and r_val >= HRS_THRESHOLD:
                    is_successful = True
                ...
                records.append({
                    ...
                    "phase": phase,
                    "state_after": ("LRS" if r_val <= LRS_THRESHOLD else
                                    "HRS" if r_val >= HRS_THRESHOLD else "MID"),
                    ...
                    "successful_cycles": successful_cycles   # successful cycles counter
                })
            ...
            def _run_pair(set_pat, set_last, reset_pat, reset_last):
                ...
                while True:
                    ...
                    # --- Logic for the outcome of the SET pulse ---

                    # Case 1: Successful SET
                    if r <= LRS_THRESHOLD:
                        if r < LRS_OVERDRIVE_THRESHOLD:
                            # It worked, but switched too hard. Correct vset down.
                            if vset > 1.0:
                        ...
                        break # Success, exit SET loop

                    # Case 2: Failed SET, stuck in OVERDRIVE HRS
                    elif r > HRS_OVERDRIVE_THRESHOLD:
                        # Correct vreset down as intended.
                        if vreset > 0.6:
                            vreset = round(vreset - correction_step, ROUND_DIGITS)
                            logger.warning("Stuck in High HRS (>40kΩ) after SET; vreset ↓ to %.3f", vreset)
                        else:
                            logger.warning("Stuck in High HRS (>40kΩ) after SET; vreset capped at %.3f", vreset)
                        # Also apply the standard correction for a failed SET.
                        if vset < 2.0:
                            vset = round(vset + correction_step, ROUND_DIGITS)
                            set_failure += 1
                            logger.warning("Bootstrap SET → still in HRS; vset ↑ to %.3f", vset)
                        else:
                            set_failure = max_set_fail
                            logger.warning("Bootstrap SET → not in LRS; max vset reached, vset %.3f", vset)

                    # Case 3: Failed SET, stuck in normal HRS
                    else: # This covers r > LRS_THRESHOLD and r <= HRS_OVERDRIVE_THRESHOLD
                        if vset < 2.0:
                            vset = round(vset + correction_step, ROUND_DIGITS)
                            set_failure += 1
                    ...
                    # Check for termination conditions after any failure
                ...
                # ---- RESET until HRS ----
                # Perform the first RESET attempt before starting the correction loop.
                ...
                # Now, start the loop to check the result and apply corrections if needed.
                while r < HRS_THRESHOLD:
                    if reset_failure >= max_reset_fail:
                        logger.error("Bootstrap RESET failed ≥ %d times → STOP", max_reset_fail)
                        failed = True
                        break

                    # Apply correction for failed RESET (resistance too low)
                    if vreset < 1.2:
                        vreset = round(vreset + correction_step, ROUND_DIGITS)
                        reset_failure += 1
                        logger.warning("Bootstrap RESET → not back to HRS; vreset ↑ to %.3f", vreset)
                    else:
                        reset_failure = max_reset_fail # Force failure
                        logger.warning("Bootstrap RESET → not back to HRS; max vreset reached, vreset %.3f", vreset)
                        continue # Re-check failure condition at top of loop

                    # Rerun the measurement with the new voltage
                    logger.info("Bootstrap RESET round (Retry): %d cycle(s)", cycles_reset)
                    r = call_ppg_with_params(padname, reset_pat, delay_block, 1, reset_last, vset, vreset, recordlen_small)
                    overall_cycles += cycles_reset
                    _log_round("RESET", cycles_reset, r, used_reps=1, capped=False)

                # After the loop, if we haven't failed, we are in HRS. Check for overdrive.
                if not failed and r > HRS_OVERDRIVE_THRESHOLD:
                    if vreset > 0.6:
                        vreset = round(vreset - correction_step, ROUND_DIGITS)
            ...
            while not failed:
                ...
                if successful_cycles >= target_total_cycles:
                    logger.info("Successful cycles goal reached %d → STOP", successful_cycles)
                    failed = True
                    break
                ...
                set_successful = LRS_OVERDRIVE_THRESHOLD < r <= LRS_THRESHOLD
                if set_successful:
                    successful_cycles += cycles_this_round
                ...
                records.append({
                    ...
                    "phase": "SET",
                    "state_after": "LRS" if r <= LRS_THRESHOLD else "HRS",
                ...
                if set_successful:                    
                    ...
                    while r < HRS_THRESHOLD and reset_failure < max_reset_fail:
                        ...
                        reset_successful = r >= HRS_THRESHOLD
                        if reset_successful:
                            successful_cycles += cycles_this_round
                        ...
                        records.append({
                            ...
                            "num_cycles": int(cycles_this_round),
                            "phase": "RESET",
                            "state_after": "HRS" if r >= HRS_THRESHOLD else "LRS",
                            "resistance_ohm": float(r),
                            "vset": float(vset), "vreset": float(vreset),
                            "set_failure": int(set_failure), "reset_failure": int(reset_failure),
                            "repetitions": int(desired_reps),  
                    ...
                    if r >= HRS_THRESHOLD:    
                        ...
                        if r > HRS_OVERDRIVE_THRESHOLD and vreset > 0.6:
                    ...
                    if r < LRS_OVERDRIVE_THRESHOLD:
                        # This block handles when resistance is TOO LOW.
                        if vset > 1:
                            vset = round(vset - correction_step, ROUND_DIGITS)
                            logger.warning("High LRS (<1kΩ) → vset ↓ to %.3f; cycle_exp reset to 0.0", vset)
                        else:
                            logger.warning("High LRS (<1kΩ), but vset cant be lowered, capped at %.3f", vset)
                        # We still count this as a set failure, but of a different kind.
                        set_failure += 1
                        n = 0.0

                    elif r > LRS_THRESHOLD:
                        # This block handles when resistance is TOO HIGH.
                        if vset < 2:
                            vset = round(vset + correction_step, ROUND_DIGITS)
